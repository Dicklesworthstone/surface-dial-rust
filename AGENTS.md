...
- **Binary:** `surface-dial` (default run target)

### Key Dependencies

| Crate | Purpose |
|-------|---------|
| `hidapi` | USB HID communication with Surface Dial hardware |
...
| `core-foundation` + `core-graphics` | macOS platform integration (conditional) |
| `tempfile` | Temporary files for testing (dev) |

### Platform Support

This is a macOS-focused daemon for controlling system volume via a Microsoft Surface Dial over USB HID. Platform modules exist for Linux, Windows, and mock backends but macOS is the primary target.
...
mcp__morph-mcp__warp_grep(
  repoPath: "/data/projects/surface-dial-rust",
  query: "How does the HID device polling loop work?"
)
```
